...
public class LocalIndexMapper {
	...
	public LocalIndexMapper(boolean isConstructor, boolean isStatic, SootMethod method, List<LValue> definedLocals,
			List<Local> locals) {
	...
	/**
	 * Retrieves the local index for a given Immediate (usually a Local). If the
	 * Local is not yet mapped to an index, a new index is generated.
	 *
	 * @param immediate The immediate value (typically a Local) for which to
	 *                  retrieve the index.
		...
		if (immediate instanceof Local) {
			...
			if(!localIndexMap.containsKey(local)) {
				if(local.getName().contains("this")) {
					...
					int maxValue = localIndexMap.values().stream().mapToInt(Integer::intValue).max().orElse(0);
			...
			throw new UnsupportedOperationException("Unsupported Immediate type: " + immediate.getClass().getName());
		}
	...
	public String getAllMappedIndices() {
		...
		for (Map.Entry<Local, Integer> entry : localIndexMap.entrySet()) {
			indicesInfo.append("Local: ").append(entry.getKey() == null ? "this" : entry.getKey().toString())
					.append(", Index: ").append(entry.getValue()).append("\n");
		}
	...
	private static int extractIndexFromLocalName(String localName) {
	    ...
	    java.util.regex.Matcher matcher = java.util.regex.Pattern.compile(primaryPattern).matcher(localName);
	    if (matcher.find()) {
	        try {
