...
public class InvokeStatementConverter extends ConversionUtil {
	...
	static Logger logger = LoggerFactory.getLogger(InvokeStatementConverter.class);
        ...
        public void convertInvokeStmt(JInvokeStmt stmt, MethodVisitor mv, LocalIndexMapper indexMapper, Boolean isConstructor, Boolean isStatic) {
        ...
        if (invokeExpr instanceof JStaticInvokeExpr) {
            ...
            handleSpecialInvokeExpr((JSpecialInvokeExpr) invokeExpr, mv, indexMapper, asmMethodDescriptor,isConstructor,isStatic);
        } else if (invokeExpr instanceof JInterfaceInvokeExpr) {
            handleInterfaceInvokeExpr((JInterfaceInvokeExpr) invokeExpr, mv, indexMapper, asmMethodDescriptor);
        } else if (invokeExpr instanceof JVirtualInvokeExpr) {
            handleVirtualInvokeExpr((JVirtualInvokeExpr) invokeExpr, mv, indexMapper, asmMethodDescriptor, isStatic, isConstructor);
        } else if (invokeExpr instanceof JDynamicInvokeExpr) {
            handleDynamicInvokeExpr((JDynamicInvokeExpr) invokeExpr, mv, indexMapper);
        } else {
            throw new UnsupportedOperationException("Unsupported invoke expression type: " + invokeExpr.getClass().getName());
        }
    }

    private void handleStaticInvokeExpr(JStaticInvokeExpr invokeExpr, MethodVisitor mv, String asmMethodDescriptor, LocalIndexMapper indexMapper) {
    ...
    private void handleVirtualInvokeExpr(JVirtualInvokeExpr invokeExpr, MethodVisitor mv, LocalIndexMapper indexMapper, String asmMethodDescriptor, Boolean isStatic, Boolean isConstructor) {
        ...
        if ( !isConstructor && !isStatic) {
        	 if (base != null &&  base.getUses().count() > 0) {
    ...
    private void handleDynamicInvokeExpr(JDynamicInvokeExpr invokeExpr, MethodVisitor mv, LocalIndexMapper indexMapper) {
    ...
    private void loadArguments(List<Immediate> args, MethodVisitor mv, LocalIndexMapper indexMapper) {
        for (Immediate arg : args) {
            if (arg instanceof Local) {
            	...
            	int index = indexMapper.getLocalIndex(arg); //indexMapper.isStatic() ? indexMapper.getIndexFromDef((Local)arg) :
            		if(!indexMapper.isStackVariable((Local) arg)) {
    ...
    private void loadIntegerConstant1(int value, MethodVisitor mv) {
        if (value >= -1 && value <= 5) {
            mv.visitInsn(Opcodes.ICONST_0 + value);
        } else if (value >= Byte.MIN_VALUE && value <= Byte.MAX_VALUE) {
            mv.visitIntInsn(Opcodes.BIPUSH, value);
